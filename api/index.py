...
def index_html():
    ...
    Synthesized from WeEvolveIT (floating pill capsule navbar, card candle-glow, ambient WebGL cosmos),
...
<head>
    ...
    <!-- Editorial & Monospace Typography: Geist, JetBrains Mono, & Crimson Pro -->
    ...
    <!-- Luxury Editorial Design System (WeEvolveIT + Spyker Cars + Creche Tank) -->
    <style>
        :root {{
            /* Obsidian Cyber Void (Dark Mode) */
            --bg-base: #050811;
            --bg-card: rgba(10, 15, 28, 0.78);
            --bg-card-hover: rgba(16, 24, 45, 0.90);
            --bg-sub: rgba(0, 0, 0, 0.42);
            --border-hairline: rgba(255, 255, 255, 0.08);
            --border-glow: rgba(0, 240, 255, 0.35);
            --border-hover: rgba(0, 240, 255, 0.65);
            --text-head: #FFFFFF;
            --text-body: #CBD5E1;
            --text-sub: #94A3B8;
            --cyan-accent: #00F0FF;
            --cyan-bg: rgba(0, 240, 255, 0.12);
            --cyan-border: rgba(0, 240, 255, 0.30);
            --indigo-accent: #818CF8;
            --emerald-accent: #00FF9D;
            --amber-accent: #FFB800;
            --rose-accent: #F43F5E;
            --vignette-start: rgba(5, 8, 17, 0.55);
            --vignette-end: rgba(5, 8, 17, 0.92);
            --glow-color: rgba(0, 240, 255, 0.16);
            --card-shadow: 0 25px 50px -12px rgba(0, 0, 0, 0.7);
            --pill-bg: rgba(10, 15, 28, 0.85);
            --pill-border: rgba(255, 255, 255, 0.12);
            --input-bg: rgba(0, 0, 0, 0.5);
            --dock-bg: rgba(10, 15, 28, 0.90);
            --radar-grid: rgba(255, 255, 255, 0.08);
            --radar-poly: rgba(0, 240, 255, 0.26);
            --radar-stroke: #00F0FF;
            --node-fill: #0A0F1C;
            --bar-track: #1E293B;
        }}
        ...
        html.light {{
            /* Pristine Alabaster Studio (Bright Mode) */
            --bg-base: #F8FAFC;
            --bg-card: rgba(255, 255, 255, 0.92);
            --bg-card-hover: #FFFFFF;
            --bg-sub: rgba(241, 245, 249, 0.92);
            --border-hairline: rgba(15, 23, 42, 0.09);
            ...
            --cyan-border: rgba(2, 132, 199, 0.32);
            --indigo-accent: #4F46E5;
            --emerald-accent: #059669;
            --amber-accent: #D97706;
            --rose-accent: #E11D48;
            --vignette-start: rgba(248, 250, 252, 0.55);
        ...
        * {{
        ...
        .vignette-overlay {{
            ...
            z-index: 1;
            pointer-events: none;
            background: radial-gradient(ellipse at center, var(--vignette-start) 0%, var(--vignette-end) 88%);
            transition: background 0.3s ease;
        }}

        /* WeEvolveIT + Spyker Luxury Glass Card with Candle Glow */
        .luxury-card {{
            position: relative;
        ...
        /* WeEvolveIT Floating Pill Navigation */
        .floating-pill-nav {{
            position: fixed;
            top: 1.25rem;
            left: 50%;
        ...
        /* Spyker Editorial Chapter Markers */
        .chapter-header {{
            display: flex;
...
<body class="min-h-screen relative selection:bg-cyan-500 selection:text-black">
    ...
    <!-- Full-Screen 3D Three.js WebGL Cosmos -->
    <canvas id="webglCanvas"></canvas>

    <!-- Depth-of-Field Vignette Layer -->
    <div class="vignette-overlay"></div>

    <!-- ===================================================================== -->
    <!-- WE-EVOLVE-IT INSPIRED FLOATING CAPSULE NAVIGATION -->
    <!-- ===================================================================== -->
    <nav class="floating-pill-nav flex items-center justify-between gap-4 max-w-4xl w-[92%] sm:w-auto">
        <!-- Brand -->
        <a href="#heroSection" class="flex items-center gap-2.5 group shrink-0">
            ...
            <span class="font-bold tracking-tight c-head text-sm hidden sm:inline">CODEDNA</span>
        </a>

        <!-- Live Status Beacon -->
        <div class="hidden lg:flex items-center gap-2 px-2.5 py-1 rounded-full text-[11px] font-mono" style="background-color: var(--cyan-bg); color: var(--cyan-accent); border: 1px solid var(--cyan-border);">
            <span class="w-1.5 h-1.5 rounded-full pulse-beacon" style="background-color: var(--cyan-accent);"></span>
            <span>EMPIRICAL ENGINE</span>
        </div>

        <!-- Section Navigation Links (Desktop) -->
        <div class="hidden md:flex items-center gap-1 text-xs font-medium c-body font-mono">
            ...
            <a href="#dnaSection" class="px-2.5 py-1 rounded-full hover:opacity-80 transition-opacity">04 DNA</a>
        ...
        <div class="flex items-center gap-2 shrink-0">
            <!-- Search ⌘K -->
            <button onclick="toggleCmdPalette()" class="flex items-center gap-1.5 px-2.5 py-1 text-xs font-mono c-body luxury-sub rounded-full hover:border-cyan-500 transition-colors" title="Search (⌘K)">
                <span>Search</span>
                <kbd class="px-1 py-0.2 rounded bg-black/10 dark:bg-white/10 text-[9px] c-sub">⌘K</kbd>
            </button>

            <!-- Theme Toggle -->
            <button onclick="toggleTheme()" class="w-7 h-7 rounded-full flex items-center justify-center c-body luxury-sub hover:opacity-80 transition-colors" title="Toggle Theme (Dark Void / Studio Light)" aria-label="Toggle Theme">
    ...
    <!-- Main Editorial Container (Elevated Above 3D Background) -->
    <main class="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-20">

        <!-- ===================================================================== -->
        <!-- CHAPTER 00 // THE ATELIER: HERO & INSTANT DECODER -->
        <!-- ===================================================================== -->
        <section id="heroSection" class="pt-8 pb-14 text-center max-w-4xl mx-auto">
            <!-- Micro-Badge (WeEvolveIT Monospace Pill) -->
            <div class="inline-flex items-center gap-2 px-3.5 py-1 rounded-full text-[11px] font-mono mb-6 shadow-sm" style="background-color: var(--cyan-bg); border: 1px solid var(--cyan-border); color: var(--cyan-accent);">
                <span class="w-1.5 h-1.5 rounded-full pulse-beacon" style="background-color: var(--cyan-accent);"></span>
                <span>CHAPTER 00 // THE ATELIER • DEVELOPER GENOME SEQUENCING</span>
            </div>

            <!-- Spyker Dramatic Typography Headline -->
            <h1 class="text-4xl sm:text-6xl lg:text-7xl font-black c-head tracking-tight leading-[1.04] mb-3">
                CODEDNA
            </h1>

            <p class="text-xl sm:text-3xl font-extrabold bg-gradient-to-r from-cyan-400 via-indigo-400 to-purple-400 bg-clip-text text-transparent mb-5">
                Decode your developer digital twin.
            </p>

            <p class="text-xs sm:text-sm c-body font-normal leading-relaxed mb-8 max-w-2xl mx-auto">
                Transform public GitHub telemetry into an empirical developer genome: multi-vector behavioral scoring, Scikit-Learn archetype clustering, and predictive career role readiness.
            </p>

            <!-- Holographic Search Terminal Input -->
            <form onsubmit="handleAnalyzeSubmit(event)" class="luxury-card p-2 sm:p-2.5 flex flex-col sm:flex-row gap-2 max-w-xl mx-auto mb-6">
                ...
                <button type="submit" class="px-6 py-2.5 bg-gradient-to-r from-cyan-500 to-blue-600 hover:opacity-90 text-white dark:text-black font-bold text-xs rounded-xl flex items-center justify-center gap-2 transition-all shadow-md cursor-pointer group shrink-0">
                    <span>Decode Genome ⚡</span>
                    <span class="group-hover:translate-x-1 transition-transform">→</span>
                </button>
            </form>

            <!-- Quick-Switch Benchmark Personas (Shruti Rai Featured) -->
            <div class="flex flex-wrap items-center justify-center gap-2 text-xs font-mono">
                ...
                <button onclick="loadProfile('alex-datascientist')" class="preset-btn px-2.5 py-1.5 rounded-full luxury-sub c-body hover:border-cyan-500 transition-all">Dr. Alex (Data Science)</button>
                <button onclick="loadProfile('elena-mlops')" class="preset-btn px-2.5 py-1.5 rounded-full luxury-sub c-body hover:border-cyan-500 transition-all">Elena (MLOps)</button>
                <button onclick="loadProfile('marcus-fullstack')" class="preset-btn px-2.5 py-1.5 rounded-full luxury-sub c-body hover:border-cyan-500 transition-all">Marcus (Full-Stack)</button>
                <button onclick="loadProfile('sophia-systems')" class="preset-btn px-2.5 py-1.5 rounded-full luxury-sub c-body hover:border-cyan-500 transition-all">Sophia (Rust)</button>
            </div>
        </section>
        ...
        <!-- ===================================================================== -->
        <section id="architectureSection" class="mb-14">
            <div class="chapter-header">
                <div>
                    <span class="chapter-num">01 / EMPIRICAL ARCHITECTURE</span>
                    <h2 class="text-2xl sm:text-3xl font-black c-head tracking-tight mt-1">How CodeDNA Decodes Any Developer</h2>
                </div>
                <span class="chapter-meta hidden sm:inline">3-Stage Synthesis Pipeline</span>
            </div>

            <!-- 3-Stage Visual Pipeline Diagram (Zero Essay Text) -->
                ...
                <div class="luxury-card p-6 flex flex-col justify-between">
                    <div>
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <span class="text-[10px] font-mono px-2 py-0.5 rounded c-sub luxury-sub uppercase">Input Telemetry</span>
                        </div>
                        <h3 class="text-base font-bold c-head mb-2">Ingest Git Artifacts</h3>
                        <p class="text-xs c-body leading-relaxed mb-4">
                            Audits public repositories, commit cadence, language byte-mass, dependencies, and code hygiene.
                        </p>
                    </div>
                    <div class="pt-3 border-t flex items-center justify-between text-[11px] font-mono c-accent" style="border-color: var(--border-hairline);">
                        <span>✓ 16+ Repos Audited</span>
                        <span class="c-sub">Stage 1 of 3</span>
                    </div>
                </div>
                ...
                <div class="luxury-card p-6 flex flex-col justify-between">
                    <div>
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <span class="text-[10px] font-mono px-2 py-0.5 rounded c-sub luxury-sub uppercase">Biological Engine</span>
                        </div>
                        <h3 class="text-base font-bold c-head mb-2">Biological Code Genome</h3>
                        <p class="text-xs c-body leading-relaxed mb-4">
                            Calculates anti-burstiness consistency (CV), Shannon polyglot entropy, and Scikit-Learn KMeans archetypes.
                        </p>
                    </div>
                    <div class="pt-3 border-t flex items-center justify-between text-[11px] font-mono" style="border-color: var(--border-hairline); color: var(--indigo-accent);">
                        <span>✓ Anti-Burstiness Filtering</span>
                        <span class="c-sub">Stage 2 of 3</span>
                    </div>
                </div>
                ...
                <div class="luxury-card p-6 flex flex-col justify-between">
                    <div>
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <span class="text-[10px] font-mono px-2 py-0.5 rounded c-sub luxury-sub uppercase">Trajectory</span>
                        </div>
                    ...
                    <div class="pt-3 border-t flex items-center justify-between text-[11px] font-mono" style="border-color: var(--border-hairline); color: var(--emerald-accent);">
                        <span>✓ Predictive Career Radar</span>
                        <span class="c-sub">Stage 3 of 3</span>
                    </div>
                </div>
            </div>

            <!-- Visual Contrast: Vanity GitHub vs CodeDNA Intelligence -->
            <div class="luxury-card p-6 sm:p-7">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <!-- Vanity Stats -->
                    <div class="p-4 rounded-xl border" style="background-color: rgba(244, 63, 94, 0.07); border-color: rgba(244, 63, 94, 0.22);">
                        ...
                        <ul class="text-xs font-mono c-body space-y-2">
                            ...
                            <li class="flex items-center gap-2"><span class="text-rose-500">✕</span> Green Calendar Grid (rewards superficial typo fixes)</li>
                            <li class="flex items-center gap-2"><span class="text-rose-500">✕</span> Star Counts (measures social buzz, not architecture)</li>
                            <li class="flex items-center gap-2"><span class="text-rose-500">✕</span> Zero Career Insight (no actionable trajectory)</li>
                        </ul>
                    </div>

                    <!-- CodeDNA Intelligence -->
        ...
        <!-- ===================================================================== -->
        <section id="overviewSection" class="mb-14">
            <div class="chapter-header">
                <div>
                    <span class="chapter-num">02 / GENOMIC PHENOTYPE</span>
                    <h2 class="text-2xl sm:text-3xl font-black c-head tracking-tight mt-1">Developer Identity & Empirical Phenotype</h2>
                </div>
                <span class="chapter-meta hidden sm:inline">Shruti Rai Featured Dossier</span>
            </div>

            <!-- Identity Dossier Card -->
            <div class="luxury-card p-6 sm:p-8 mb-6">
                <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                    <div class="flex items-center gap-5">
                        <div class="relative">
                            ...
                            <span class="absolute -bottom-1 -right-1 w-4 h-4 rounded-full bg-emerald-500 border-2" style="border-color: var(--bg-card);" title="Verified Live Developer"></span>
                        </div>
                        <div>
                    ...
                    <!-- Telemetry Stats -->
                    <div class="flex items-center gap-4 w-full md:w-auto border-t md:border-t-0 md:border-l pt-4 md:pt-0 md:pl-6" style="border-color: var(--border-hairline);">
                        <div class="text-center px-3">
                            <div id="statRepos" class="text-2xl font-bold font-mono c-head">16</div>
                            <div class="text-[10px] uppercase font-mono c-sub">Repositories</div>
                        </div>
                        <div class="text-center px-3">
                            <div id="statFollowers" class="text-2xl font-bold font-mono c-head">17</div>
                            <div class="text-[10px] uppercase font-mono c-sub">Followers</div>
                        </div>
                        <div class="text-center px-3">
                            <div id="statAge" class="text-2xl font-bold font-mono c-accent">2.4y</div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-6">
                ...
                <div class="lg:col-span-5 luxury-card p-6 sm:p-8 flex flex-col items-center justify-center text-center relative overflow-hidden">
                    <div class="text-xs font-mono uppercase tracking-widest c-sub font-bold mb-3 flex items-center gap-1.5">
                        <span class="w-1.5 h-1.5 rounded-full pulse-beacon" style="background-color: var(--cyan-accent);"></span>
                        ...
                        <div class="orbit-container">
                            <svg class="orbit-ring-svg" viewBox="0 0 160 160">
                                <g class="orbit-particles-group">
                                    <circle cx="80" cy="10" r="3" fill="#06B6D4" filter="url(#particleGlow)"/>
                ...
                <div class="lg:col-span-7 luxury-card p-6 sm:p-8 flex flex-col justify-between">
                    <div class="flex items-center justify-between mb-4">
        ...
        <!-- ===================================================================== -->
        <section id="twinSection" class="mb-14">
            <div class="chapter-header">
                <div>
                    <span class="chapter-num">03 / THE DIGITAL TWIN</span>
                    <h2 class="text-2xl sm:text-3xl font-black c-head tracking-tight mt-1">7D Mathematical Fingerprint & Archetype</h2>
                </div>
                <span class="chapter-meta hidden sm:inline">Scikit-Learn KMeans Clustering</span>
            </div>

            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                <!-- 7D Radar Visualization (7 Cols) -->
                <div class="lg:col-span-7 luxury-card p-6 sm:p-8">
                    <div class="flex items-center justify-between mb-4">
                        <div>
                        ...
                        <span class="px-2.5 py-1 rounded-full text-xs font-mono border" style="background-color: rgba(99, 102, 241, 0.15); color: var(--indigo-accent); border-color: rgba(99, 102, 241, 0.35);">Vector Space</span>
                    </div>
                    ...
                    <!-- Hover Evidence Box -->
                    <div id="radarTooltip" class="mt-3 p-3 rounded-xl luxury-sub text-xs font-mono c-body flex items-center justify-between">
                ...
                <div class="lg:col-span-5 luxury-card p-6 sm:p-8 flex flex-col justify-between">
                    <div>
                        ...
                        <div class="p-4 rounded-xl luxury-sub mb-4">
        ...
        <!-- ===================================================================== -->
        <section id="dnaSection" class="mb-14">
            <div class="chapter-header">
                <div>
                    <span class="chapter-num">04 / TECHNOLOGICAL ECOSYSTEM</span>
                    <h2 class="text-2xl sm:text-3xl font-black c-head tracking-tight mt-1">Technology DNA & Stack Momentum</h2>
                </div>
                <span class="chapter-meta hidden sm:inline">Hierarchical Graph & Shifts</span>
            </div>

            <div class="luxury-card p-6 sm:p-8">
                <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 items-center">
                    <!-- Interactive Graph (8 cols) -->
                    <div class="lg:col-span-8 luxury-sub rounded-2xl p-4 relative overflow-hidden h-96 flex items-center justify-center">
                        <svg id="dnaNetworkSvg" class="w-full h-full" viewBox="0 0 500 300">
                            <!-- Rendered dynamically -->
                        </svg>
                        <!-- Node popover -->
                        <div id="dnaNodeTooltip" class="hidden absolute bottom-3 left-3 right-3 p-3 luxury-card rounded-xl text-xs font-mono c-body flex items-center justify-between shadow-xl">
        ...
        <!-- ===================================================================== -->
        <section id="rhythmSection" class="mb-14">
            <div class="chapter-header">
                <div>
                    <span class="chapter-num">05 / TEMPORAL CHRONOMETRY</span>
                    <h2 class="text-2xl sm:text-3xl font-black c-head tracking-tight mt-1">Growth Velocity & 7x24 Focus Rhythm</h2>
                </div>
                <span class="chapter-meta hidden sm:inline">Cadence Analysis</span>
            </div>
            ...
            <div class="luxury-card p-6 sm:p-8 mb-6">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                    <div>
                        <h3 class="text-base font-bold c-head tracking-tight">Developer Growth Velocity (DGV)</h3>
                        <p class="text-xs c-sub font-mono">Multi-year trajectory tracking depth, complexity, and technology expansion</p>
                    </div>
                ...
                <div class="w-full h-60 luxury-sub rounded-2xl p-4 relative flex items-center justify-center mb-4">
                    <svg id="velocitySvg" class="w-full h-full" viewBox="0 0 700 200">
                        <!-- Rendered dynamically -->
                    </svg>
                </div>

                <div id="velocitySnapshotCard" class="p-4 rounded-xl luxury-sub text-xs font-mono flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
                    <div>
            ...
            <div class="luxury-card p-6 sm:p-8">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                    <div>
                        <h3 class="text-base font-bold c-head tracking-tight">7x24 Focus Cadence Punchcard</h3>
                        <p class="text-xs c-sub font-mono">Hourly commit density mapping deep-work intervals</p>
                    </div>
                ...
                <div class="luxury-sub rounded-2xl p-4 sm:p-6 mb-4 overflow-x-auto">
                ...
                <div id="rhythmInsightBox" class="p-3.5 rounded-xl luxury-sub text-xs font-mono c-body">
                    <!-- Insight text -->
                </div>
            </div>
        </section>
        ...
        <!-- ===================================================================== -->
        <section id="projectsSection" class="mb-14">
            <div class="chapter-header">
                <div>
                    <span class="chapter-num">06 / AUDITED REPOSITORIES</span>
                    <h2 class="text-2xl sm:text-3xl font-black c-head tracking-tight mt-1">Repository Atelier & Architecture</h2>
                </div>
                <span class="chapter-meta hidden sm:inline">16 Repositories Audited</span>
            </div>
        ...
        <!-- ===================================================================== -->
        <section id="careerSection" class="mb-14">
            <div class="chapter-header">
                <div>
                    <span class="chapter-num">07 / SYNTHETIC CAREER RADAR</span>
                    <h2 class="text-2xl sm:text-3xl font-black c-head tracking-tight mt-1">Career Role Leaderboard & Next Best Skill</h2>
                </div>
                <span class="chapter-meta hidden sm:inline">Vector Space Similarity</span>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div class="lg:col-span-7 luxury-card p-6 sm:p-8 flex flex-col justify-between">
                    <div>
                        <div class="flex items-center justify-between mb-4">
                            <div>
                                <h3 class="text-base font-bold c-head tracking-tight">Role Match Leaderboard</h3>
                ...
                <div class="lg:col-span-5 space-y-6">
                    ...
                    <div class="luxury-card p-6" style="background: linear-gradient(135deg, var(--cyan-bg) 0%, var(--bg-card) 100%);">
                        <div class="flex items-center justify-between mb-3">
                        ...
                        <div class="p-3 rounded-xl luxury-sub text-xs font-mono flex items-center gap-2" style="color: var(--emerald-accent);">
        ...
        <section class="mb-14">
            <div class="chapter-header">
                <div>
                    ...
                    <h2 class="text-2xl sm:text-3xl font-black c-head tracking-tight mt-1">Developer API & JSON Mart</h2>
                </div>
                <span class="chapter-meta hidden sm:inline">REST Specification</span>
            </div>
            ...
            <div class="luxury-card p-6 sm:p-8">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-4">
                    ...
                    <div class="flex items-center gap-2">
                        <button onclick="copyApiUrl()" class="px-3 py-1.5 rounded-full luxury-sub text-xs font-mono c-body hover:opacity-80 transition-opacity flex items-center gap-1.5">
                            <span id="copyBtnText">Copy Endpoint</span>
                        </button>
                        <a id="viewJsonLink" href="/api/profile?username=shrutirai29" target="_blank" class="px-3 py-1.5 rounded-full text-xs font-mono c-accent font-bold transition-opacity hover:opacity-80" style="background-color: var(--cyan-bg); border: 1px solid var(--cyan-border);">
                            View Raw JSON ↗
                        </a>
                    </div>
                </div>
    ...
    <!-- Floating visionOS Cyber Dock (Bottom Shortcuts) -->
    <div class="cyber-dock hidden sm:flex items-center gap-1 font-mono text-xs">
        <button onclick="jumpToSection('#heroSection')" class="px-2.5 py-1 rounded-full hover:opacity-75 transition-opacity" title="Hero">Atelier</button>
        <button onclick="jumpToSection('#architectureSection')" class="px-2.5 py-1 rounded-full hover:opacity-75 transition-opacity c-accent font-bold" title="Architecture">Pipeline</button>
    ...
    <div id="cmdPalette" class="hidden fixed inset-0 z-50 bg-black/75 backdrop-blur-sm flex items-start justify-center pt-24 p-4">
        <div class="max-w-lg w-full luxury-card shadow-2xl p-4 overflow-hidden">
            ...
            <div class="space-y-1 font-mono text-xs max-h-64 overflow-y-auto" id="cmdResults">
                <div onclick="jumpToSection('#architectureSection')" class="p-2 rounded-lg hover:opacity-80 cursor-pointer flex items-center justify-between c-body luxury-sub">
                    <span>01 / Empirical Architecture</span>
                    <span class="c-sub">Pipeline</span>
                </div>
                <div onclick="jumpToSection('#overviewSection')" class="p-2 rounded-lg hover:opacity-80 cursor-pointer flex items-center justify-between c-body luxury-sub">
                    <span>02 / Genomic Phenotype</span>
                    <span class="c-sub">Profile</span>
                </div>
                <div onclick="jumpToSection('#twinSection')" class="p-2 rounded-lg hover:opacity-80 cursor-pointer flex items-center justify-between c-body luxury-sub">
                    <span>03 / The Digital Twin</span>
                    <span class="c-sub">7D Radar</span>
                </div>
                <div onclick="jumpToSection('#dnaSection')" class="p-2 rounded-lg hover:opacity-80 cursor-pointer flex items-center justify-between c-body luxury-sub">
                    <span>04 / Technological Ecosystem</span>
                    <span class="c-sub">DNA Graph</span>
                </div>
                <div onclick="jumpToSection('#rhythmSection')" class="p-2 rounded-lg hover:opacity-80 cursor-pointer flex items-center justify-between c-body luxury-sub">
                    <span>05 / Temporal Chronometry</span>
                    <span class="c-sub">Rhythm</span>
                </div>
                <div onclick="jumpToSection('#projectsSection')" class="p-2 rounded-lg hover:opacity-80 cursor-pointer flex items-center justify-between c-body luxury-sub">
                    <span>06 / Audited Repositories</span>
                    <span class="c-sub">Projects</span>
                </div>
                <div onclick="jumpToSection('#careerSection')" class="p-2 rounded-lg hover:opacity-80 cursor-pointer flex items-center justify-between c-body luxury-sub">
    ...
    <script>
        ...
        function renderProfile(p) {{
            ...
            momList.innerHTML = (p.momentum || []).map(m => `
                <div class="p-2.5 rounded-xl luxury-sub flex items-center justify-between text-xs font-mono">
                    <div>
            ...
            projGrid.innerHTML = (p.projects || []).map(pr => `
                <div class="luxury-card p-5 flex flex-col justify-between group">
                    <div>
        ...
        function renderRadarChart(dims) {{
            ...
            const polyFill = isLight ? 'rgba(2, 132, 199, 0.22)' : 'rgba(0, 240, 255, 0.26)';
            const polyStroke = isLight ? '#0284C7' : '#00F0FF';
            const nodeStroke = isLight ? '#FFFFFF' : '#050811';
        ...
        function renderDnaNetwork(nodes) {{
            ...
            const isLight = document.documentElement.classList.contains('light');
            const nodeFill = isLight ? '#FFFFFF' : '#0A0F1C';
        ...
        function renderVelocityTimeline(timeline) {{
            ...
            const nodeBg = isLight ? '#FFFFFF' : '#050811';
            ...
            chipBox.innerHTML = timeline.map(t => `
                <button onclick="selectTimelineYear('${{t.year}}')" class="px-2.5 py-1 rounded-full luxury-sub hover:border-cyan-500 c-body font-mono transition-colors">${{t.year}}</button>
            `).join('');
        ...
        function renderPunchcard(rhythm) {{
            ...
            if (rhythm.insight) {{
                document.getElementById('rhythmInsightBox').innerHTML = `<strong>Focus Rhythm Observation:</strong> ${{rhythm.insight}}`;
            }}
        }}
        ...
        function renderCareerBars(careers) {{
            ...
            container.innerHTML = careers.map(c => `
                <div class="p-3.5 rounded-xl luxury-sub">
        ...
        function renderSkillGaps(gaps) {{
            ...
            if (gaps.critical) {{
                html += `
                    <div class="p-3 rounded-xl border" style="background-color: rgba(244, 63, 94, 0.08); border-color: rgba(244, 63, 94, 0.28);">
            ...
            if (gaps.important) {{
                html += `
                    <div class="p-3 rounded-xl border" style="background-color: rgba(245, 158, 11, 0.08); border-color: rgba(245, 158, 11, 0.28);">
        ...
        window.addEventListener('DOMContentLoaded', () => {{
            ...
            // Render default profile (Shruti Rai)
